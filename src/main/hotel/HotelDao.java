...
public class HotelDao implements Dao<Hotel> {
    ...
    public boolean update(Hotel item) {
        String sql = "UPDATE Hotel SET name = ?, address = ?, phoneNumber = ?, mobilePhoneNumber = ?, singleRoom = ?, doubleRoom = ?, tripleRoom = ?, otherRoom = ?, roomCount = ?, bedCount = ?, isHotel = ?, photoWay = ? WHERE id = ?";
    ...
    public void insert(Hotel item) {
        String sql = "INSERT INTO Hotel(name,address,phoneNumber,mobilePhoneNumber,singleRoom,doubleRoom,tripleRoom,otherRoom,roomCount,bedCount,isHotel,photoWay) VALUES(?,?,?,?,?,?,?,?,?,?,?,?)";
